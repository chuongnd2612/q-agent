      ...
      {chatOpen && spec && (
        ...
        >
          <motion.aside
            ...
            className="pointer-events-auto absolute right-0 top-0 flex h-full w-[480px] max-w-[96vw] flex-col border-l border-bd2 shadow-[0_0_60px_-10px_var(--shadow)]"
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-bd px-4 py-3">
              <span className="accent-gradient flex h-8 w-8 items-center justify-center rounded-lg">
                <Sparkles size={16} color="var(--pOn)" />
              </span>
              <div className="min-w-0">
                <div className="text-[13px] font-bold leading-tight">Q-Agent</div>
                <div className="truncate font-mono text-[11px] text-txt4">{spec.filename}</div>
              </div>
              <select
                value={model}
                onChange={(e) => setModel(e.target.value)}
                className="ml-auto rounded-lg border border-bd2 bg-card px-2 py-1 text-[11px] text-txt3"
              ...
              <button
                onClick={closeChat}
                className="flex h-7 w-7 items-center justify-center rounded-lg text-txt4 hover:bg-card3"
            ...
            <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-4">
              {messages.length === 0 && (
                <div className="px-2 py-8 text-center text-[12.5px] text-txt4">
            ...
            {/* Composer */}
            <div className="relative border-t border-bd p-3">
              {/* @-mention dropdown: the run's specs the reviewer can embed as context. */}
              {mention && mentionMatches.length > 0 && (
                <div className="absolute inset-x-3 bottom-full mb-2 max-h-56 overflow-y-auto rounded-xl border border-bd2 bg-pop py-1 shadow-[0_20px_50px_-15px_var(--shadow)]">
                  <div className="px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-txt4">
                  ...
                  {mentionMatches.map((s) => (
                    <button
                      ...
                      className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-[12px] hover:bg-card3"
                    >
                      <Sparkles size={12} className="shrink-0 text-ps-text" />
                      <span className="truncate font-mono text-txt3">{s.filename}</span>
                    </button>
                  ))}
                </div>
              )}
              {guidanceMode && (
                ...
                <div
                  className="mb-2 rounded-lg px-2.5 py-1.5 text-[11px] text-ps-text-text"
              ...
              <div className="mb-2 flex flex-wrap gap-1.5">
                {quickActions.map((q) => (
                  <button
                    ...
                    disabled={busy}
                    onClick={() => send(q)}
                    className="rounded-full border border-bd2 bg-inset px-2.5 py-1 text-[11px] text-txt3 hover:bg-card3 disabled:opacity-40"
                  >
                    {q}
                  </button>
                ))}
              </div>
              <div className="flex items-end gap-2 rounded-xl border border-bd2 bg-card3 p-2">
                <textarea
                  ref={textareaRef}
                  value={input}
                  onChange={(e) => {
                  ...
                  className="max-h-32 flex-1 resize-none bg-transparent px-1 text-[13.5px] text-txt outline-none placeholder:text-txt4"
                />
                <button
                  onClick={() => send(input)}
...
function UserBubble({ text }: { text: string }) {
  return (
    <div className="flex justify-end">
      <div className="accent-gradient max-w-[85%] rounded-[14px_14px_5px_14px] px-3 py-2 text-[12.5px] text-p-on">
        {text}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex gap-2.5">
      <span className="accent-gradient mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg">
        <Sparkles size={13} color="var(--pOn)" />
      </span>
      <div className="min-w-0 flex-1 space-y-2">
        {m.thinking ? (
          <span className="inline-flex gap-1 text-txt4" aria-label={t("automation.chat.thinking")}>
            <Dot /> <Dot /> <Dot />
          </span>
        ) : m.error ? (
...
function DiffSnippet({ prev, next }: { prev: string; next: string }) {
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-bd2 bg-code p-2 font-mono text-[11px] leading-relaxed">
      {gone.map((l, i) => (
        <div key={`r${i}`} style={{ color: "var(--danger)", background: "var(--dangerTint)" }}>
          - {l}
        </div>
      ))}
      {added.map((l, i) => (
        <div key={`a${i}`} style={{ color: "var(--ok)", background: "var(--okTint)" }}>
          + {l}
        </div>
      ))}
    </div>
...
function AppliedCard({ prev, next, onUndo }: { prev: string; next: string; onUndo: () => void }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-success/30 bg-ok/10 px-3 py-2 text-[12px]">
      <span className="text-success">
        {count === 0 ? t("automation.chat.appliedNoChanges") : t("automation.chat.appliedLines", { count })}
      </span>
      {count > 0 && (
        <button onClick={onUndo} className="ml-auto flex items-center gap-1 text-txt3 hover:text-txt" title={t("automation.chat.undo")}>
...
function RevertedCard({ onReapply }: { onReapply: () => void }) {
  const { t } = useTranslation("pipeline");
  return (
    <div className="flex items-center gap-2 rounded-lg border border-bd2 bg-inset px-3 py-2 text-[12px] text-txt4">
      {t("automation.chat.editReverted")}
      <button onClick={onReapply} className="ml-auto text-txt3 hover:text-txt">
