...
#: Reason recorded when a paused live-authoring session (#619) was never continued
#: and its pause window elapsed, so the Chrome window and temp workdir the device
#: was holding are released and the case becomes re-triggerable.
EXPIRED_AUTHORING_PAUSE_REASON = (
    "Live authoring was paused and never continued — the session expired and was torn down."
)
...
def recover_orphaned_authoring(db: Session) -> tuple[int, int]:
    ...
    0. **Expire forgotten pauses (#619).** A ``paused`` session is the one state
       this sweep must NOT treat as stranded — the device is holding a live
       Chrome, a live temp workdir and a live ``CLAUDE_CONFIG_DIR`` open for it,
       and it survives an API restart exactly like a ``running`` one. But a pause
       nobody continues would keep that browser on the user's desktop forever, so
       a pause older than ``PAUSE_EXPIRES_AFTER`` is dropped here and its spec
       falls through to pass 2 and becomes re-triggerable. (The device learns to
       tear down from its own resume poll, which is the path that actually closes
       Chrome; this is the backstop for a device that never comes back.)
    ...
    expired = agent_authoring_service.expire_stale_pauses(db)
    ...
    for _run_id, _case_id, session_id in expired:
        audit_service.record(
            category="automation",
            actor_type="system",
            action="Expired a forgotten live-authoring pause",
            target=session_id,
            status="warning",
            meta=EXPIRED_AUTHORING_PAUSE_REASON,
        )
    if survivors:
        logger.info(
    ...
    return len(reset), dropped + len(expired)
