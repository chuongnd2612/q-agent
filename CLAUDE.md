...
## Deploying / rebuilding the running stack

- **The running deployment is the `emesoft` suite, orchestrated from the *other* repo** — `D:\chuong.nguyen\claude-projects\emehub\docker-compose.suite.yml`, which `extends` this repo's `docker-compose.yml` for the `qagent-api` / `qagent-web` services. So the standing "rebuild after shipping" step is **not** `docker compose up -d --build` in `q-agent/`: that derives the project name from the directory, builds a **second, parallel stack**, collides on the db port, fails — and still **exits 0**, so it looks like it worked.
- **Rebuild with the suite script**, which is there precisely so the long form is not retyped: `cd ../emehub && ./suite.sh` (or `suite.ps1`). Interpolation like `${QAGENT_SECRET_KEY:?…}` resolves against the *project's* environment, not the file each service came from, so **all three** `.env` files must be passed (`.env`, `dagent/.env`, `../q-agent/.env`) — the script does that.
- **Do not rebuild just `qagent-api qagent-web` on their own.** Recreating only those two reconciles the suite networks and gives them new IPs, while `emesoft-edge` is left running with a published-port mapping pointing at the old address. nginx inside edge keeps answering `200`, but from the host the connection is accepted and then hangs with no response — which reads exactly like "EmeHub is down" and is not. If it happens, `docker restart emesoft-edge` restores it; no data is touched. Rebuilding the whole suite avoids it, because edge is recreated in the same pass.
