...
- **The suite is very nearly green: 1 test fails on `master` (#469).** Baseline it *before* you start and compare **by name**, not by count, and don't assume a failure is yours. The one known failure is `test_claude_creds.py::test_upload_and_delete_own_credentials` — a stale exact-dict `==` assertion against a response that has since grown `accountEmail` / `accountOrg` / `status`.
- **Treat "known red" as a bug, not a baseline.** The count went 22 → 18 → 1, and each drop was a real coverage hole that the "permanently red, just baseline it" framing had been hiding:
